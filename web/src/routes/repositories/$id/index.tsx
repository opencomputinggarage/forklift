...
function Settings({ repo, setRepo, canWrite }: { repo: Repository; setRepo: (r: Repository) => void; canWrite: boolean }) {
  ...
  return (
    <>
      ...
      {canWrite && repo.type !== "group" && <button className={buttonVariants()} onClick={save}>Save changes</button>}
      ...
      <div className="panel danger" style={{ marginTop: 18 }}>
        ...
        {repo.type !== "group" && (
          <>
            ...
            <button className={buttonVariants({ variant: "destructive" })} onClick={() => setConfirmPurge(true)}>Purge all artifacts</button>
        ...
        <button className={buttonVariants({ variant: "destructive" })} onClick={() => setConfirmDelete(true)}>Delete repository</button>
      </div>
      </>
      )}
    </>
            ...
            <tr key={a.path}>
              ...
              {canDelete && (
                <td style={{ textAlign: "right" }}>
                  <button className={buttonVariants({ variant: "outline" })} onClick={() => setDeleting(a)}>Delete</button>
                </td>
              )}
            </tr>
          ))}
  ...
  return (
    <div className="panel">
      ...
      <div className="inline" style={{ marginTop: 12 }}>
        ...
        <button className={buttonVariants({ variant: "outline" })} type="button" onClick={refresh}>Refresh</button>
      </div>
      {error && <div className="error">{error}</div>}
      <table style={{ marginTop: 12 }}>
        <thead>
      ...
      {data && data.count > AUDIT_PAGE_SIZE && (
        <div className="inline" style={{ marginTop: 12 }}>
          <button className={buttonVariants({ variant: "outline" })} type="button" disabled={offset === 0}
            onClick={() => setOffset(Math.max(0, offset - AUDIT_PAGE_SIZE))}>Newer</button>
          <button className={buttonVariants({ variant: "outline" })} type="button" disabled={offset + AUDIT_PAGE_SIZE >= data.count}
